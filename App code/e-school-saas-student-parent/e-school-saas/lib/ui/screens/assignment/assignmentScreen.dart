...
class _AssignmentScreenState extends State<AssignmentScreen> {
  ...
  bool _showUploadAssignmentButton() {
    ...
    //they can also undo the assignment resubmission if it's resubmitted & extended due date is not passed
    if (assignmentStatusKey == resubmittedKey) {
      //calculate extended due date for resubmission
      DateTime extendedDueDate = dueDate.add(
        Duration(
          days: submittedAssignment.extraDaysForResubmission,
        ),
      );
      //if extended due date has not passed, show undo button
      if (currentDayDateTime.compareTo(extendedDueDate) != 1) {
        isUndoAssignmentSubmissionButtonToBeShown = true;
        return true;
      }
      //if extended due date has passed, hide button
      return false;
    }
    ...
    //Handle rejected assignments - allow resubmission based on dates
    if (assignmentStatusKey == rejectedKey) {
      //Calculate final deadline for resubmission
      //If extra days provided, extend the due date, otherwise use original due date
      DateTime finalDeadline = dueDate;
      if (submittedAssignment.extraDaysForResubmission > 0) {
        finalDeadline = dueDate.add(
          Duration(days: submittedAssignment.extraDaysForResubmission),
        );
      }

      //Allow upload if current time is before or equal to final deadline
      if (currentDayDateTime.compareTo(finalDeadline) != 1) {
        return true; //Show upload button
      }
      //Final deadline has passed
      return false;
    }

    //Handle unsubmitted assignments
    //If not submitted yet (no submission or invalid status)
    ...
    //For any other status, show upload button if within due date
    return true;
  }
