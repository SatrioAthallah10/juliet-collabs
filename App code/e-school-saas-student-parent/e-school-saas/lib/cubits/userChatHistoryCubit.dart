...
class UserChatHistoryCubit extends Cubit<UserChatHistoryState> {
  ...
  void updateLastMessage(
    ...
    if (state is UserChatHistoryFetchSuccess) {
      ...
      // Parse the API date format "dd-MM-yyyy HH:mm"
      DateTime? apiDateTime;
      try {
        final parts = chatContact.updatedAt.split(' ');
        if (parts.length == 2) {
          ...
          if (dateParts.length == 3 && timeParts.length == 2) {
            ...
            apiDateTime = DateTime(year, month, day, hour, minute);
          }
        }
      } catch (e) {
      ...
      if (apiDateTime != null && lastMessageTime.isAfter(apiDateTime)) {
        final updatedContact = chatContact.copyWith(
