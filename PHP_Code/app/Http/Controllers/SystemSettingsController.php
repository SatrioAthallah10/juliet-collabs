...
class SystemSettingsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            EnvSet::save();
            //    dd($data);
            $inserted = $this->systemSettings->upsert($data, ["name"], ["data", "type"]);

            // Log the updates for logos
            foreach ($data as $item) {
                if (isset($item['type']) && $item['type'] === 'file' && in_array($item['name'], ['horizontal_logo', 'vertical_logo', 'favicon', 'login_page_logo', 'student_web_background_image'])) {
                    \Illuminate\Support\Facades\Log::channel('logo_updates')->info('Custom Logo Updated (System)', [
                        'type' => $item['name'],
